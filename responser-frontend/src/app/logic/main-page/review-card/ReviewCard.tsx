...
const ReviewCard: React.FC<ReviewCardProps> = (props: ReviewCardProps) => {
    const {review, currentUser, createLike, updateLike, removeLike, disableReactions} = props;
    const {user, rating, text, creationDate, reviewLikes, webResource: {resourceType, url}} = review;
    ...
    return (
        <div className={className}>
            <div className="card-header">
                ...
                <div className="web-resource-info">
                    <span className="resource-type">{getResourceTypeLabel()}: </span>
                    <Link className="resource-url" target="_blank" href={url}>{url}</Link>
                </div>
